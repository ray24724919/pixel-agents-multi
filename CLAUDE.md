# Pixel Agents Multi — Compressed Reference

VS Code extension with embedded React webview: pixel art office where AI agents (Claude Code + Codex sessions) are animated characters. Fork of `pablodelucca/pixel-agents` (`raychen.pixel-agents-multi`), heavily extended: dual-provider adoption, project-room campus, Agent Center, handoff (交接) workflow.

> **Current state, roadmap scoreboard, and working rules live in [`docs/HANDOVER.md`](docs/HANDOVER.md).** Read it before starting work. All user data lives under **`~/.pixel-agents-multi/`** (NOT the legacy `~/.pixel-agents/`, which exists only as a one-time migration source).
...
src/                          — Extension backend (Node.js, VS Code API)
  ...
  configPersistence.ts        — User-level config file I/O (~/.pixel-agents-multi/config.json), external asset directories
  layoutPersistence.ts        — User-level layout file I/O (~/.pixel-agents-multi/layout.json), migration, cross-window watching
  handoffArtifacts.ts         — Handoff (交接) core: draft/sidecar model, work packages, executor launch/cancel, completion detection, multi-repo scan
  handoffLaunchEvidence.ts    — Evidence gate: an opened terminal is NOT a started session; wait for transcript/rollout/hook proof (12s timeout)
  timelineEvents.ts           — TimelineEventKind union + record shapes (adding a kind: ALSO update PixelAgentsViewProvider postHandoffTimelineEvent union + title/severity chains)
  timelineStore.ts / usageHistoryStore.ts — Persisted timeline/usage history (user-level data dir)
...
## Fork Subsystems (the "Multi" layer — absent from upstream)

**Dual-provider external-agent lifecycle** — ray's agents are ALL adopted external sessions (`isExternal: true`, no extension-owned terminals). Policy is **deletion-based on all three paths** (adopt / restore / reap): an agent lives while its transcript (Claude JSONL) or rollout (Codex) exists and its Codex thread isn't archived; idle time NEVER kills it. "Deleted" = `isExternalTranscriptGone()` in `agentManager.ts` (**ENOENT/ENOTDIR only** — a transient stat error must never reap; two reapers share it: `reapStaleExternalAgents` + fileWatcher's `startStaleExternalAgentCheck`). Three duplicate guards: `reapDuplicateTerminalAgents` (by terminal), `reapDuplicateExternalAgents` (by session), restore-time `claimedTerminals`/`claimedSessions`; path identity is win32-lowercased (`jsonlPathKey`, `codexPathKey`) because drive/hash-dir casing differences caused repeated dup bugs. Claude cowork/local-agent-mode sandboxes (`local-agent-mode-sessions` in path) are rejected by raw adopters AND the scanner's failed-rebase fallback (`isLocalAgentModeSandboxPath`).

**Codex provider** (`server/src/providers/file/codex/codex.ts`, ~970 lines): threads from `~/.codex/state_5.sqlite` `threads` table (`archived=0` = live), rollouts under `~/.codex/sessions/`. `selectCodexAdoptionCandidates` adopts any non-archived non-subagent thread in an allowed cwd (no recency gate). Kill = `terminateCodexThreadProcess` + `archiveCodexThread`. `startCodexCwdPoll` binds a launched terminal to its thread/rollout when it appears.

**Handoff (交接) — the product spine** (`src/handoffArtifacts.ts` + `handoffLaunchEvidence.ts` + webview `handoffArtifactLibraryModel.ts`/`useHandoffWorkflow.ts`/`HandoffPanels.tsx`): draft Markdown + `.handoff.json` sidecar in `docs/agent-handoffs/` → work package (`docs/roadmap/supervision/work-packages/handoffs/`) → Launch Codex/Claude executor terminal (evidence-gated: no transcript/rollout/hook proof in 12s ⇒ kill the orphan terminal + roll back, `handoff.executor_launch_timed_out`) → executor writes report to `docs/roadmap/supervision/reports/` on a branch → completion auto-rescanned when the linked executor settles (`selectHandoffAutoRefreshTargets`, dual-provider via statusGroup). Dispatch statuses `draft|ready|dispatched|completed|blocked`; "Cancel executor" (Work Queue) = `clearHandoffExecutorLink` (dispatched→ready, drops link) + best-effort kill. Repo root resolves PER SESSION (`resolveGitRepoRoot` from the source agent's projectDir; library scans all roots via `scanHandoffArtifactsAcrossRoots`) so handoffs work in an office-only window and write to the right repo. ⚠️ The three doc paths above are LIVE runtime data — never mass-move/edit them. Adding a timeline kind = 3 places (timelineEvents.ts union + provider `postHandoffTimelineEvent` union + its title/severity chains).

**Campus / project rooms** (`webview-ui/src/office/`: `projectRoomGeneration.ts` orchestrator + `campusBounds.ts` + `lobbyLounge.ts` + `projectRooms.ts` + `generationShared.ts`/`geometry.ts`): each project (agent `projectDir`) gets a generated room in a work corridor around a public lobby; new rooms fill bays centre-out (`orderBayColsCenterOut`); the lobby widens right-anchored only when bay capacity < expected rooms (`widenWorkCorridorRightAnchored`); camera centers on campus on entry (`computeCampusCenterPan`). New rooms are stamped VERBATIM from `PROJECT_ROOM_TEMPLATE` (extracted from ray's hand-designed room; bump `rev` to propagate template edits via `migrateExistingRoomsToTemplate`). Generation is **frozen + additive-only**: existing rooms/furniture are never moved or deleted; auto-reclaim of empty rooms is DISABLED (`applyVacancyLifecycle` no-ops without an explicit cutoff) — rooms persist until ray deletes them. `ProjectRoom` one-time stamps (`loungeRev`, `loungeSpanWidth`) suppress regeneration; **any new persisted room field MUST be added to `normalizeProjectRoom`'s field-copy or it's silently stripped every provision**.

**Agent Center** (`webview-ui/src/components/agentCenter/` — 15 modules after the v1.3.34 split of a 5.8k-line component; state in `useAgentCenterState.ts`, handoff flows in `useHandoffWorkflow.ts`): pages Office / Agents / Usage / Timeline. Agents page metrics: SHOWN/VISIBLE = webview `agents.length` (only shrinks via `agentClosed`). Remove UX: office × or Agents-page "Remove…" → Hide (filter, keeps running) / Archive (permanent remove, transcript kept, never re-adopted) / Kill (terminate process; hidden for external Claude — no terminal handle).

**Timeline / usage / replay + teams**: persisted event history (`src/timelineStore.ts`, `usageHistoryStore.ts` → user data dir) rendered by `timelinePageModel.ts` / `usageHistoryPageModel.ts` / `TimelineReplayPanel`; teams detection in `server/src/teamProvider.ts` + `claudeTeamProvider.ts` (lead/teammate links on AgentState).
...
**Sound notifications**: Ascending two-note chime (E5 → E6) via Web Audio API plays when waiting bubble appears (`agentStatus: 'waiting'`). `notificationSound.ts` manages AudioContext lifecycle; `unlockAudio()` called on canvas mousedown to ensure context is resumed (webviews start suspended). Toggled via "Sound Notifications" checkbox in Settings modal. Enabled by default; persisted in extension `globalState` key `pixel-agents-multi.soundEnabled`, sent to webview as `settingsLoaded` on init.
...
**Layout model**: `{ version: 1, cols, rows, tiles: TileType[], furniture: PlacedFurniture[], tileColors?: FloorColor[] }`. Grid dimensions are dynamic (not fixed constants). Persisted via debounced saveLayout message → `writeLayoutToFile()` → `~/.pixel-agents-multi/layout.json`.
